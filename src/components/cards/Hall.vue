            ...
            <v-card-subtitle>
                <v-tooltip text="Price in USD">
                    <template v-slot:activator="{ props }">
                        <span v-bind="props" class='me-3'><v-icon icon="attach_money" color="green" /> {{ hall.price
                        }}</span>
                    </template>
                </v-tooltip>
                <v-tooltip text="Size in square feet">
                    <template v-slot:activator="{ props }">
                        <span v-bind="props"> <v-icon icon="square_foot" color="orange"/> {{ hall.size }} </span>
                    </template>
                </v-tooltip>
            </v-card-subtitle>
            <v-card-subtitle>
                <v-tooltip text="Parking">
                    <template v-slot:activator="{ props }">
                        <v-icon v-bind="props" class="me-2" v-if="hall.parking == 'true'" icon="local_parking"
                            color="primary" />
                    </template>
                </v-tooltip>
                <v-tooltip text="Free Wi-Fi">
                    <template v-slot:activator="{ props }">
                        <v-icon v-bind="props" class="me-2" v-if="hall.internet == 'true'" icon="wifi" color="primary" />
                    </template>
                </v-tooltip>
                <v-tooltip text="Pets friendly">
                    <template v-slot:activator="{ props }">
                        <v-icon v-bind="props" class="me-2" v-if="hall.pets == 'true'" icon="pets" color="primary" />
                    </template>
                </v-tooltip>
                <v-tooltip text="Accessible for everyone">
                    <template v-slot:activator="{ props }">
                        <v-icon v-bind="props" class="me-2" v-if="hall.dissability == 'true'" icon="accessible"
                            color="primary" />
                    </template>
                </v-tooltip>
            </v-card-subtitle>
