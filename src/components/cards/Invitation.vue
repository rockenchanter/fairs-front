    ...
    <v-card>
        ...
        <v-card-text class="mt-3">
            ...
            <div v-if="item.invitation || (!item.stall_id && item.status == 1)">
                <RouterLink :to="{ name: 'profile', params: { id: item.fair.organizer_id } }" class="font-weight-bold">
            ...
            <!-- request -->
            <div v-else>
                <RouterLink :to="{ name: 'profile', params: { id: item.company.exhibitor_id } }" class="font-weight-bold">
                    {{ item.company.exhibitor.name }} {{ item.company.exhibitor.surname }}
                </RouterLink>
                representing
                <RouterLink :to="{ name: 'companies-show', params: { id: item.company.id } }" class="font-weight-bold">
                    {{ item.company.name }}
                </RouterLink>
        ...
        <v-card-actions>
            ...
            <v-tooltip text="Accept invitation/request" v-if="!item.invitation && item.status == 0" >
                <template v-slot:activator="{ props }">
                    <ResponsiveBtn v-bind="props" color="green" prepend text="accept"
                        icon="done" @click="acceptRequest" />
                </template>
            </v-tooltip>
            ...
            <v-dialog width="auto" v-model="stallDialog" v-else>
                ...
                <v-card>
                    <v-container>
                        ...
                        <v-card-text>
                            <v-row>
                                <v-col cols="12" sm="6" md="3" v-for="stall in availableStalls" :key="stall.id"
                                    ...
                                    <StallCard :stall="stall" :color="selectedItem.id == stall.id ? 'indigo-lighten-4' : ''"
                        ...
                        <v-card-actions>
                            <v-spacer />
                            <ResponsiveBtn v-bind="props" prepend text="send" :disabled="!selectedItem.id && !respondable"
                                @click="acceptInvitation" icon="check" color="green" />
                        </v-card-actions>
                    </v-container>
                </v-card>
            </v-dialog>
            ...
            <v-tooltip text="Reject invitation/request">
                <template v-slot:activator="{ props }">
                    <ResponsiveBtn v-bind="props" :disabled="!respondable" @click="declineInvitation" color="red" prepend
                        text="decline" icon="delete_forever" />
                </template>
            </v-tooltip>
        </v-card-actions>
    </v-card>
