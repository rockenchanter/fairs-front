...
<template>
    <v-form @submit.prevent="sendForm">
        ...
        <v-row v-if="props.id">
            <v-col>
                <v-file-input :error-messages="errors.path" label="Image" prepend-icon="" prepend-inner-icon="photo_camera"
                    v-model="item.path" name="path" accept="image/*" />
                <v-text-field :error-messages="errors.description" label="Description" v-model.trim="item.description"
                    name="description" />
        ...
        <v-row v-else justify="start">
            <v-col>
                ...
                <v-row>
                    <v-col cols="8">
                        <v-text-field :error-messages="errors.description" label="Description"
                            v-model.trim="item.description" name="description" /></v-col>
                    <v-col>
                        <v-btn-group>
