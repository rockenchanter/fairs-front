...
const sendForm = async (event) => {
  ...
  if (item.value.id) {
    ...
    if (data.errors) {
      errors.value = data.errors.address
    } else ds.showAlert('success', 'Address has been updated')
  } else {
  ...
  <v-form @submit.prevent="sendForm">
    ...
    <v-row>
      <v-col>
        ...
        <v-btn-group>
          <v-btn
            :icon="item.id ? 'update' : 'save'"
            :color="item.id ? 'green' : 'primary'"
            type="submit"
          />
          <v-btn
            v-if="item.id && !disableDelete"
            icon="delete"
            color="red"
            @click="deleteAddress()"
          />
