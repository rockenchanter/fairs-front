...
const sendForm = async (event) => {
    ...
    if (item.value.id) {
        ...
        if (data.errors) {
            ...
            errors.value = {}
            ds.showAlert('success', 'Company has been updated')
            emit('update', data.id)
        }
    } else {
                ...
                <v-form @submit.prevent="sendForm">
                    ...
                    <v-window v-model="tab">
                        <v-window-item value="one">
                            <v-row>
                                <v-col cols="12" sm="4"><v-text-field :error-messages="errors.name" v-model.trim="item.name"
                                        name="name" label="Name" /></v-col>
                                <v-col cols="12" sm="4"><v-select :error-messages="errors.industries"
                                        v-model="item.industries" name="industry" :items="ds.industries" multiple
                                        item-title="name" item-value="id" label="Industries">
                                ...
                                <v-col v-if="!item.id" cols="12" sm="4"><v-file-input :error-messages="errors.image"
                                        v-model="item.image" prepend-icon="" prepend-inner-icon="photo_camera"
                                        accept="image/*" name="image" label="Logo" /></v-col>
                            </v-row>

                            <v-row v-if="!item.id">
                                <v-col>
                                    <v-text-field v-model.trim="address.street" :error-messages="addr_errors.street"
                                        label="Street" name="street" />
                                </v-col>
                                <v-col>
                                    <v-text-field v-model.trim="address.zipcode" :error-messages="addr_errors.zipcode"
                                        label="Zipcode" name="zipcode" />
                                </v-col>
                                <v-col>
                                    <v-text-field v-model.trim="address.city" :error-messages="addr_errors.city"
                                        label="City" name="city" />
                                </v-col>
                            </v-row>

                            <v-row>
                                <v-col><v-textarea :error-messages="errors.description" v-model.trim="item.description"
                                        name="description" label="Description" /></v-col>
                            </v-row>

                            <v-row class="pb-2">
                                <v-col class="text-right">
                                    <ResponsiveBtn icon="save" color="primary" type="submit" text="save" />
                                    <ResponsiveBtn class="ms-2" v-if="item.id" icon="delete" color="red" text="delete"
                                        @click="confDialog = true" />
                                    <ConfirmationDialog :visible="confDialog" @accept="deleteCompany"
                        ...
                        <v-window-item value="two">
                            <v-row>
                                <v-col cols="4" v-for="address in item.addresses" :key="address.id">
                                    <AddressForm :address="address" @delete="deleteAddress"
                                        :disable-delete="item.addresses.length == 1" />
                                </v-col>
                                <v-col cols="4">
                                    <AddressForm @create="addAddress" :company_id="item.id" />
                                </v-col>
                            </v-row>
                        </v-window-item>
                    </v-window>
