...
<template>
    <v-form @submit.prevent="sendForm">
        ...
        <!-- update -->
        <div v-if="item.id">
            <v-row><v-col><v-img :src="item.image" max-height="200px" /></v-col></v-row>
            <v-row>
                <v-col cols="4">
                    <v-switch color="primary" v-model="item.electricity" label="Electricity" name="electricity" />
                </v-col>
                <v-col cols="4">
            ...
            <v-row>
                <v-col cols="8"> <v-text-field :error-messages="errors.size" type="number" v-model.number="item.size"
                        label="Size" name="size" /></v-col>
                <v-col>
                    <v-btn-group>
        ...
        <div v-else>
            <v-row>
                <v-col>
                    <v-text-field :error-messages="errors.size" type="number" v-model.number="item.size" label="Size"
                        name="size" />
                    <v-text-field v-if="!item.id" type="number" :error-messages="errors.max_amount"
                        v-model.number="item.max_amount" label="Amount" name="max_amount" />
                    <v-file-input v-if="!item.id" :error-messages="errors.image" accept="image/*" prepend-icon=""
                        prepend-inner-icon="photo_camera" v-model="item.image" label="Image" name="image" />
                </v-col>
            </v-row>
            <v-row>
                <v-col cols="4">
