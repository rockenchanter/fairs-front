                ...
                <v-data-table density="compact" :headers="headers" :items="halls">
                    <template v-slot:top>
                        <v-toolbar color="primary" title="Halls" rounded>
                            <v-spacer />
                            <v-btn @click="openHallDialog(null)" text="create hall" color="white" variant="elevated" />
                        </v-toolbar>
                    </template>

                    <template v-slot:item.public="{ item }">
                        <v-icon :icon="item.selectable.public == 'true' ? 'done' : 'close'"
                            :color="item.selectable.public == 'true' ? 'green' : 'red'" />
                    </template>
                    <template v-slot:item.actions="{ item }">
                        <RouterLink :to="{ name: 'halls-show', params: { id: item.value } }"><v-icon icon="visibility"
                                color="blue" /></RouterLink>
                        <v-icon class="mx-2" icon="edit" color="orange" @click="openHallDialog(item)" />
                        <v-icon icon="delete" color="red" @click="confDialog = true" />
                            <ConfirmationDialog @accept="deleteHall(item)" @decline="confDialog = false" :visible="confDialog"/>
                    </template>
                </v-data-table>
            </v-col>
        </v-row>
